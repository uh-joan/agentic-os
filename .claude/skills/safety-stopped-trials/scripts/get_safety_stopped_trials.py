...
def get_safety_stopped_trials(indication, condition_subtypes=None):
    ...
    Args:
        indication (str): Disease or therapeutic area (e.g., "diabetes", "NSCLC", "obesity")
        condition_subtypes (dict, optional): Mapping of subtype labels to keyword lists.
            Example: {'Type 1': ['type 1', 't1d'], 'Type 2': ['type 2', 't2d']}
            If None, all trials categorized as "Other/Unspecified"
    ...
    Returns:
        dict: Contains:
            ...
            - categorizations: Breakdown by safety reason, phase, condition subtype
            - notable_drugs: Scored drug failures with actual safety reasons
            - summary: Human-readable summary with "Why Stopped" examples
    """

    print(f"Searching for {indication} trials stopped due to safety concerns...")
    ...
    for i, (keyword, severity_score) in enumerate(safety_keywords.items(), 1):
        ...
        while True:
            ...
            # CRITICAL: This is the line that was hardcoded to "diabetes" - now parameterized
            query = f'{indication} AND (AREA[OverallStatus]TERMINATED OR AREA[OverallStatus]WITHDRAWN OR AREA[OverallStatus]SUSPENDED) AND AREA[WhyStopped]{keyword}'
    ...
    # Categorize trials
    categorizations = categorize_trials(trials, condition_subtypes)
    ...
    # Generate summary
    summary = generate_summary(trials, categorizations, total_count, scored_drugs, indication)
...
def categorize_trials(trials, condition_subtypes=None):
    """Categorize trials by phase, condition subtype (if provided), and status.
    ...
    for trial in trials:
        ...
        # Categorize by condition subtype (generic, based on parameter)
            ...
            for subtype_label, keywords in condition_subtypes.items():
                if any(keyword in conditions_lower for keyword in keywords):
                    categorizations['by_condition_subtype'][subtype_label] = \
                        categorizations['by_condition_subtype'].get(subtype_label, 0) + 1
                    subtype_found = True
                    break  # First match wins

            if not subtype_found:
                categorizations['by_condition_subtype']['Other/Unspecified'] = \
                    categorizations['by_condition_subtype'].get('Other/Unspecified', 0) + 1
        else:
...
def generate_summary(trials, categorizations, total_count, scored_drugs, indication):
    """Generate formatted summary with notable drugs table."""

    lines = [
        "=" * 80,
        f"{indication.upper()} TRIALS STOPPED DUE TO SAFETY CONCERNS",
        "=" * 80,
        "",
    ...
    if categorizations['by_condition_subtype']:
        lines.append("Condition Subtype Breakdown:")
        for dtype, count in categorizations['by_condition_subtype'].items():
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("\nExamples:")
        print('  python get_safety_stopped_trials.py "diabetes"')
        print('  python get_safety_stopped_trials.py "NSCLC"')
        print('  python get_safety_stopped_trials.py "diabetes" \'{"Type 1": ["type 1", "t1d"], "Type 2": ["type 2", "t2d"]}\'')
        sys.exit(1)

    indication = sys.argv[1]

    # Optional: Parse condition subtypes from second argument (JSON format)
    condition_subtypes = None
    if len(sys.argv) >= 3:
        try:
            condition_subtypes = json.loads(sys.argv[2])
        except json.JSONDecodeError:
            print(f"Warning: Could not parse condition_subtypes JSON. Using None.")
            print(f"Received: {sys.argv[2]}")
            condition_subtypes = None
